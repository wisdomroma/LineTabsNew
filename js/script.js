$(function() {
	...
	countRates('#form-price_1', '#count_1');
	countRates('#form-price_2', '#count_2');
    ...
    $(".order-form").validate({
        ...
        messages: {
            ...
            phone:{
                required: "Это поле обязательно для заполнения",
                digits: "Только из цифр",
                minlength: "Логин должен быть минимум 7 символов"
            },
            mail:{
                required: "Это поле обязательно для заполнения"
            },
...
function countRates(form_price, count_input) {

	var order_price = $(form_price+' .js-order-price').html(),
		delivery_price = $(form_price+' .js-delivery-price').html(),
		num = $(count_input),
		total_price = $(form_price+' .js-total-price');

	num.on('input', function() {
		var countPrice = 0;

		for (var i = 0; i < +num.val(); i++) {
			countPrice += +order_price;
		}

		if (countPrice == 0) {
			total_price.html(+order_price + +delivery_price);
		} else {
			var result = countPrice + +delivery_price;
